...
<template>
  <div class="login-left-view">
    <div class="logo">
      <ArtLogo class="icon" size="30" />
      <h1 class="title">{{ AppConfig.systemInfo.name }}</h1>
    </div>

    <div class="left-img">
      <img :src="getPublicUrl('宣传图2.jpg')" class="promo-img" />
    </div>

    <div class="text-wrap">
      <!-- Windows 桌面版下载卡片（Tauri桌面端不显示） -->
      <div v-if="!isTauri" class="download-card" @click="handleDownload">
        <div class="download-icon">
          <i class="pi pi-desktop"></i>
        </div>
        <div class="download-info">
          <span class="download-title">Windows 桌面版</span>
          <span class="download-desc">下载安装，体验本地AI能力</span>
        </div>
...
<script setup lang="ts">
  ...
  const handleDownload = () => {
    window.open('https://oopc.tos-cn-beijing.volces.com/desk/%E4%B8%80%E4%BA%BA%E5%9B%BD%E4%BC%81%C2%B7OPC_0.1.0_x64-setup.exe', '_blank')
  }
</script>
  ...
  .login-left-view {
    ...
    .left-img {
      ...
      .promo-img {
        ...
        object-fit: contain;
        border-radius: 16px;
        border: 3px solid rgb(from $primary-base r g b / 15%);
        box-shadow:
    ...
    .text-wrap {
      ...
      .download-card {
        ...
        border: 1px solid rgb(from $primary-base r g b / 15%);
        border-radius: 12px;
        cursor: pointer;
        transition: all 0.3s;
        ...
        &:hover {
          ...
          border-color: rgb(from $primary-base r g b / 30%);
          box-shadow: 0 4px 16px rgb(from $primary-base r g b / 15%);
        ...
        .download-icon {
          ...
          align-items: center;
          justify-content: center;
          background: linear-gradient(135deg, $primary-base, var(--el-color-primary-light-3));
          border-radius: 8px;
          color: white;
          font-size: 18px;
        }
      ...
      .qrcode-wrap {
        ...
        .qrcode-img {
          ...
          height: 120px;
          border-radius: 8px;
          border: 2px solid rgb(from $primary-base r g b / 15%);
