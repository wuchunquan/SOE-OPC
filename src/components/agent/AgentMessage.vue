<template>
  <div class="agent-message" :class="messageClasses">
        ...
        <div class="role-avatar" :style="avatarStyle">
          ...
          <template v-if="message.role === 'user'">
            ...
            <template v-if="message.sender_id && message.sender_name">
              <img
                ...
                :src="baseUrl + message.sender_avatar"
            ...
            <!-- 当前用户的消息（没有 sender_id 时） -->
            <template v-else>
              <!-- 公开模式：显示访客头像 -->
              <template v-if="isPublicMode">
                <div class="avatar-placeholder">访</div>
              </template>
              <template v-else>
              <img
              ...
              />
              <div v-else class="avatar-placeholder">
                {{ (userStore.info.realName || userStore.info.userName || '用户')[0] }}
              </div>
              </template>
            </template>
          </template>
          ...
          <template v-else>
            <img
              ...
              :src="baseUrl + currentSessionInfo.role_avatar_url"
    ...
    <div class="message-content">
      <!-- 有工具调用的助手消息：时间轴视图 -->
      <template v-if="hasProcessSteps">
        <!-- 处理过程时间轴卡片 -->
        <div class="process-timeline-card" :class="{ 'is-expanded': processExpanded }">
          <div class="process-timeline-header" @click="toggleProcessExpanded">
            <div class="process-header-left">
              <i class="pi" :class="processExpanded ? 'pi-chevron-up' : 'pi-chevron-down'" style="font-size: 11px;"></i>
              <span class="process-title">处理过程</span>
              <span class="process-divider">·</span>
              <span class="process-count">{{ toolCallCount }} 次工具调用</span>
            </div>
            <div class="process-header-right">
              <span v-if="message.status === 'streaming'" class="process-status streaming">
                <i class="pi pi-spinner process-spin-icon"></i> 进行中
              </span>
              <span v-else class="process-status completed">
                <i class="pi pi-check-circle"></i> 已完成
              </span>
            </div>
          </div>
          <div class="process-timeline-collapse" :class="{ 'is-open': processExpanded }">
            <div class="process-timeline-body">
            <div v-for="(step, idx) in processStepsData.filter(step => step.block.type !== 'tool_result')" :key="'ps-' + idx" class="timeline-step" :class="{ 'is-last': idx === processStepsData.length - 1 }">
              <div class="timeline-dot" :class="'dot-' + step.block.type"></div>
              <div class="timeline-step-content">
                <!-- 中间文本（简要显示） -->
                <div v-if="step.block.type === 'text'" class="step-text-brief">
                  {{ (step.block.text || '').substring(0, 120) }}{{ (step.block.text || '').length > 120 ? '...' : '' }}
                </div>
                <!-- 工具调用 -->
                <div v-else-if="step.block.type === 'tool_use'" class="tool-use-block">
                  <ToolCallDisplay :tool-name="step.block.tool_name" :tool-input="step.block.tool_input" :streaming="message.status === 'streaming'" :working="isToolWorking(step.originalIndex)" compact />
                </div>
                <!-- 工具结果 -->
                <div v-else-if="step.block.type === 'tool_result'" class="tool-result-block">
                  <ToolResultDisplay :content="step.block.tool_result || step.block.content" compact />
                </div>
                <!-- 思考过程 -->
                <div v-else-if="step.block.type === 'thinking'" class="thinking-block">
                  <div class="thinking-card">
                    <div class="thinking-header" @click.stop="toggleThinking(step.originalIndex)">
                      <div class="thinking-header-left">
                        <i class="pi pi-sparkles thinking-icon"></i>
                        <span class="thinking-title">思考过程</span>
                        <span class="thinking-length">({{ step.block.thinking?.length || 0 }} 字符)</span>
                      </div>
                      <i class="pi thinking-chevron" :class="expandedThinking.has(step.originalIndex) ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
                    </div>
                    <div v-show="expandedThinking.has(step.originalIndex)" class="thinking-content">
                      <div class="thinking-text" v-html="formatText(step.block.thinking || '')"></div>
                    </div>
                  </div>
                </div>
                <!-- 错误 -->
                <div v-else-if="step.block.type === 'error'" class="error-block">
                  <div class="error-card">
                    <div class="error-header">
                      <ArtSvgIcon icon="ri:error-warning-line" :size="14" class="error-icon" />
                      <span class="error-title">错误</span>
                    </div>
                    <div class="error-content">{{ step.block.text }}</div>
                  </div>
                </div>
              </div>
            </div>
        ...
        <!-- 最终结果（时间轴外部醒目展示） -->
        <div v-if="finalResultBlock" class="content-block">
          <div class="text-block">
            <div v-if="hasFileReferences(finalResultBlock.block.text || '')" class="text-with-files">
              <div v-for="(part, partIndex) in parseTextWithFiles(finalResultBlock.block.text || '')" :key="'fr-' + partIndex">
                <div v-if="part.type === 'file'" class="file-reference">
                  <img v-if="isImageFile(part.path)" :src="getFileUrl(part.path)" :alt="part.path" class="file-reference-image" loading="lazy" />
                  <div v-else class="file-reference-item" @click="downloadFile(part.path)">
                    ...
                    <button v-if="isHtmlFile(part.path)" class="file-btn" title="预览" @click.stop="openHtmlPreview(part.path)"><i class="pi pi-eye"></i></button>
                    <button v-if="isPdfFile(part.path)" class="file-btn" title="预览" @click.stop="openPdfPreview(part.path)"><i class="pi pi-eye"></i></button>
                    <button v-if="!isLocal && (!isPublicMode || isHtmlFile(part.path))" class="file-btn" title="分享" @click.stop="shareFile(part.path)"><i class="pi pi-share-alt"></i></button>
                    <button class="file-btn"><i class="pi pi-download"></i></button>
                  </div>
                </div>
                <div v-else-if="part.type === 'text' && part.text.trim()" class="text-content" :ref="el => setTextContentRef(el, `final-${partIndex}`, part.text)"></div>
              </div>
            </div>
            <div v-else class="text-content" :ref="el => setTextContentRef(el, 'final', finalResultBlock.block.text || '')"></div>
          </div>
        </div>
      </template>
      ...
      <!-- 普通渲染（无工具调用的消息） -->
      <template v-else>
        <div v-for="(block, index) in message.content.filter(block => block.type !== 'tool_result')" :key="index" class="content-block">
          <div v-if="block.type === 'text'" class="text-block">
            <div v-if="hasFileReferences(block.text || '')" class="text-with-files">
              <div v-for="(part, partIndex) in parseTextWithFiles(block.text || '')" :key="partIndex">
                ...
                <div v-else-if="part.type === 'text' && part.text.trim()" class="text-content" :ref="el => setTextContentRef(el, `${index}-${partIndex}`, part.text)"></div>
              </div>
            </div>
            <div v-else class="text-content" :ref="el => setTextContentRef(el, index, block.text || '')"></div>
          </div>
          <div v-else-if="block.type === 'tool_use'" class="tool-use-block">
            <ToolCallDisplay :tool-name="block.tool_name" :tool-input="block.tool_input" :streaming="message.status === 'streaming'" :working="isToolWorking(index)" />
          </div>
          ...
          <div v-else-if="block.type === 'thinking'" class="thinking-block">
            <div class="thinking-card">
              <div class="thinking-header" @click="toggleThinking(index)">
                ...
                <i class="pi thinking-chevron" :class="expandedThinking.has(index) ? 'pi-chevron-up' : 'pi-chevron-down'"></i>
              </div>
          ...
          <div v-else-if="block.type === 'error'" class="error-block">
            <div class="error-card">
              ...
              <div class="error-content">{{ block.text }}</div>
            </div>
          </div>
        </div>
      </template>
...
import ToolCallDisplay from './common/ToolCallDisplay.vue'
import ToolResultDisplay from './common/ToolResultDisplay.vue'
import HtmlPreviewDialog from './common/HtmlPreviewDialog.vue'
import PdfPreviewDialog from './common/PdfPreviewDialog.vue'
...
interface Props {
  ...
  sessionId?: string // 会话 ID（公开模式传入）
  visitorId?: string // 访客 ID（公开模式传入）
...
/**
 * 判断工具是否正在工作（向后扫描直到找到 tool_result）
 */
const isToolWorking = (index: number): boolean => {
  if (props.message.status !== 'streaming') return false
  for (let i = index + 1; i < props.message.content.length; i++) {
    if (props.message.content[i].type === 'tool_result') return false
    if (props.message.content[i].type === 'tool_use') break
  }
  return true
}
...
// 是否有处理步骤（助手消息且包含工具调用）
const hasProcessSteps = computed(() => {
  if (props.message.role !== 'assistant') return false
  return props.message.content.some(block => block.type === 'tool_use')
})

// 处理步骤数据（带原始索引，用于 isToolWorking）
const processStepsData = computed(() => {
  if (!hasProcessSteps.value) return []
  const content = props.message.content
  // 流式中：所有内容都在时间轴内
  if (props.message.status === 'streaming') {
    return content.map((block, i) => ({ block, originalIndex: i }))
  }
  // 完成后：最后一个文本块之前的所有内容
  let lastTextIdx = -1
  ...
  const endIdx = lastTextIdx > 0 ? lastTextIdx : content.length
  return content.slice(0, endIdx).map((block, i) => ({ block, originalIndex: i }))
})

// 最终结果块（完成后的最后一个文本块）
const finalResultBlock = computed(() => {
  if (!hasProcessSteps.value) return null
  if (props.message.status === 'streaming') return null
  const content = props.message.content
  for (let i = content.length - 1; i >= 0; i--) {
    if (content[i].type === 'text' && i > 0) return { block: content[i], originalIndex: i }
  }
  return null
})
...
const toolCallCount = computed(() => {
  return props.message.content.filter(b => b.type === 'tool_use').length
})

// 时间轴展开/折叠状态
const manualProcessExpanded = ref<boolean | null>(null)
const processExpanded = computed(() => {
  if (manualProcessExpanded.value !== null) return manualProcessExpanded.value
  return props.message.status === 'streaming'
})
const toggleProcessExpanded = () => {
  manualProcessExpanded.value = !processExpanded.value
}
...
const currentSessionInfo = computed(() => {
  return props.sessionInfo || agentStore.currentSessionInfo
})
...
const roleName = computed(() => {
  if (props.message.role === 'user') {
    ...
    if (props.isPublicMode) return '访客'
    return userStore.info.realName || userStore.info.userName || '用户'
  } else {
...
const canDeleteMessage = computed(() => {
  if (props.isPublicMode) return false
  // 获取会话类型
  ...
  // 自己发送的消息没有 sender_id（或 sender_id 等于当前用户 ID）
  if (sessionType === 'chat') {
    // 如果消息没有 sender_id，说明是当前用户发送的
    if (!props.message.sender_id) {
      return true
    }
    // 如果 sender_id 等于当前用户 ID，也是自己发送的
    return props.message.sender_id === userStore.info.userId
  }

  return true
})

// 判断是否可以撤回消息（内部聊天，自己的消息，10分钟内）
const canRecallMessage = computed(() => {
  ...
  // 只能撤回自己发送的消息
  const isOwnMessage = !props.message.sender_id || props.message.sender_id === userStore.info.userId
  if (!isOwnMessage) {
    return false
  }
...
const parseTextWithFiles = (text: string): Array<{ type: 'text' | 'file'; text?: string; path?: string }> => {
  ...
  const filePattern = /@([^@\n]+?\.(png|jpg|jpeg|gif|webp|svg|pdf|txt|json|md|zip|rar|7z|xlsx|xls|docx|doc|pptx|ppt|csv|html|htm|css|js|ts|jsx|tsx|scss|less|vue|py|java|go|rs|sh|bat|yaml|yml|toml|xml|sql))/gi
...
const openHtmlPreview = (filePath: string) => {
  const sessionId = props.sessionId || agentStore.currentAiSession
  htmlPreviewUrl.value = `${baseUrl}/api/agent/sessions/${sessionId}/workspace/files/${filePath}`
...
const openPdfPreview = (filePath: string) => {
  ...
  if (props.isPublicMode) {
    pdfPreviewUrl.value = `/api/agent/public/sessions/${sessionId}/workspace/files/${filePath}?visitor_id=${props.visitorId}`
  } else {
    pdfPreviewUrl.value = `${baseUrl}/api/agent/sessions/${sessionId}/workspace/files/${filePath}`
  }
...
const getFileUrl = (path: string): string => {
  const sessionId = props.sessionId || agentStore.currentAiSession
  if (props.isPublicMode) {
    return `/api/agent/public/sessions/${sessionId}/workspace/files/${path}?visitor_id=${props.visitorId}`
  }
  return `${baseUrl}/api/agent/sessions/${sessionId}/workspace/files/${path}`
}
...
const downloadFile = async (filePath: string) => {
  try {
    const sessionId = props.sessionId || agentStore.currentSession
    if (!sessionId) {
    ...
    const downloadUrl = props.isPublicMode
      ? `/api/agent/public/sessions/${sessionId}/workspace/download?path=${encodeURIComponent(filePath)}&visitor_id=${props.visitorId}`
      : `${baseUrl}/api/agent/sessions/${sessionId}/workspace/download?path=${encodeURIComponent(filePath)}`
    ...
    if (isTauriEnv()) {
      ...
      if (!props.isPublicMode) {
        fetchOptions.headers = { 'Authorization': `Bearer ${useUserStore().accessToken}` }
      }
...
.process-timeline-header {
  ...
  // 展开时底部不圆角
  .process-timeline-card.is-expanded & {
...
.process-timeline-body {
  ...
  .process-timeline-collapse.is-open & {
    // padding-top: 4px;
    padding-bottom: 4px;
  }
}
...
// ==================== 时间轴 - 暗色模式 ====================
html.dark .process-timeline-card {
  border: 1px solid rgba(255, 255, 255, 0.08);
  background: rgba(255, 255, 255, 0.02);
}

html.dark .process-timeline-header {
  // background: rgba(255, 255, 255, 0.04);
  &:hover { background: rgba(255, 255, 255, 0.06); }
  color: #a1a1aa;
  .process-title { color: #a1a1aa; }
  .process-count { color: #a1a1aa; }
  .process-status.streaming { color: #60a5fa; }
  .process-status.completed { color: #a1a1aa; }
}
...
html.dark .step-text-brief {
  color: #a1a1aa;
}

// ==================== 时间轴 - 浅色模式 ====================
html:not(.dark) .process-timeline-card {
  border: 1px solid #e5e7eb;
  background: #ffffff;
}

html:not(.dark) .process-timeline-header {
  // background: #f5f5f5;
  &:hover { background: #efefef; }
  color: #6b7280;
  .process-title { color: #6b7280; }
  .process-count { color: #6b7280; }
  .process-status.streaming { color: #3b82f6; }
  .process-status.completed { color: #6b7280; }
}

html:not(.dark) .timeline-step::before {
  background: #e5e7eb;
}
