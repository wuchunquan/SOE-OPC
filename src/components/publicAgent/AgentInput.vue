...
const props = withDefaults(
  defineProps<{
    disabled?: boolean
    placeholder?: string
    sessionId?: string // 用于监听会话切换
    visitorId?: string // 访客ID（公开模式需要）
  }>(),
  {
    disabled: false,
    placeholder: '输入您的问题...'
...
const uploadFileToWorkspace = async (name: string, type: string, data: string) => {
  ...
  // 使用 props.sessionId 和 props.visitorId
  ...
  if (!visitorId) {
    throw new Error('没有访客ID')
  }

  // 使用公开 API 端点
  const response = await http.post({
    url: `/api/agent/public/sessions/${sessionId}/workspace/upload`,
    data: {
      name,
      type,
      data,
