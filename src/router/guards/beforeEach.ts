...
async function handleRouteGuard(
  ...
  // 3. 处理动态路由注册
  // Tauri环境下访问静态路由（如/agent）时，跳过动态路由注册
  const isAccessingStaticRoute = isStaticRoute(to.path)

  console.log('[RouteGuard] 检查动态路由:', {
    path: to.path,
    isRegistered: routeRegistry?.isRegistered(),
    isLogin: userStore.isLogin,
    isAccessingStaticRoute,
    isTauri: isTauriEnv()
  })

  if (!routeRegistry?.isRegistered() && userStore.isLogin && !isAccessingStaticRoute) {
    // 防止并发请求（快速连续导航场景）
...
async function handleLoginStatus(
  ...
  if (isSelfUseMode()) {
    try {
      const bootstrapOk = await ensureSelfUseLogin()
      if (bootstrapOk) {
        return true
      }
    } catch (error) {
      console.error('[RouteGuard] 自用模式自动登录失败:', error)
    }
...
async function handleDynamicRoutes(
  ...
  try {
    ...
    // Tauri桌面客户端检测
    const isDesktopClient = isTauriEnv()
    // 检查是否启用 AI Agent 功能
    const isAgentEnabled = import.meta.env.VITE_ENABLE_AGENT === 'true'
    // Tauri桌面客户端、手机端且开启了 Agent 功能时默认进入一人国企·OPC页面，否则使用配置的首页路径
    const defaultHomePath = ((isMobile || isDesktopClient) && isAgentEnabled) ? (isDesktopClient ? '/agent-desktop' : '/sys/agent') : (homePath.value || '/')
